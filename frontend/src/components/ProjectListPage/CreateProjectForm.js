...
class CreateProject extends Component {
  ...
  handleSubmit() {
    ...
    const toastId = toast.info(
      <div>
        <h3>
          Creating project <em>{this.state.projectName}</em>
        </h3>
        <p>
          This could take a few minutes while your project DAO is added to the
      ...
      .then(({ data }) => {
        ...
        toast.update(toastId, {
          render: (
            <div>
              <p>
                Project <em>{name}</em> created [<Link to={link}>View</Link>]
              </p>
            </div>
          ),
      ...
      .catch(err => {
        ...
        toast.update(toastId, {
          render: (
            <div>
              <p>
                Failed to create project <em>{name}</em>
              </p>
            </div>
          ),
  ...
  render() {
    ...
    return (
      <div className="Page">
        {this.state.submitting ? submitting : unsubmitted}
      </div>
    );
  }
}
